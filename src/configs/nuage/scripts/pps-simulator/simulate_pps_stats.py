...
class SimulateIKEStats(object):
    ...
    def getTunnelsAndProbesDetails(self,tunnels,nsg):
        tunnelDetails = []
        for tunnel_name in tunnels:
            tunnel = {}
            tunnel_id = tunnel_name.split("-")[1]
            tunnel['TunnelName'] = tunnel_name
            tunnel['ProbeName'] = "probe-{0}".format(tunnel_id)
            tier1_urls,tier2_urls = self.get_tier_urls()
            for url in tier1_urls:
                url['timeline'] = self.getUrlTimeline(startTime,endTime)
            for url in tier2_urls:
                url['timeline'] = self.getUrlTimeline(startTime, endTime)
            tunnel['Tier1URLs'] = tier1_urls
            tunnel['Tier2URLs'] = tier2_urls
            tunnel["TunnelID"] = uuid.uuid4()
            tunnel["ProbeID"] = uuid.uuid4()
            tunnel["SrcNSG"] = nsg['nsg_id']
            tunnel["SourceNSG"] = nsg['nsg_name']
            tunnelDetails.append(tunnel)
